...
local hookedCooldownPanels = setmetatable({}, { __mode = "k" })
local hookedEditModeManagers = setmetatable({}, { __mode = "k" })
...
local function HookCooldownViewerSettings()
	...
	if not panel or hookedCooldownPanels[panel] then
		return
	end
	hookedCooldownPanels[panel] = true
...
local function HookEditModeManager()
	...
	if not em or hookedEditModeManagers[em] then
		return
	end
	hookedEditModeManagers[em] = true
